...
de kans dat een error bestaat in de backend plugin is klein -> komt van de server -> daar heb je de logs
...
Cons:
* grafana-sdk framework supports error message strings only
...
Pros: 
* simple 
* all knowledge about backend errors is on one place 
* Frontend does not need any knowledge about error details 

Cons: 
* original error details are not available if grafana is hosted by third party 
